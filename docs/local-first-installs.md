...
1. The content script matches `<all_urls>` (`apps/extension/src/entrypoints/content.ts:10`) and
   ...
   to `GET /api/configs/lookup?url=` (`apps/extension/src/entrypoints/background.ts:64`). The
...
2. There is no `chrome.storage` usage anywhere in `apps/extension/src`. The extension holds no
   state; the only offline data is the bundled fallback (`src/lib/configs.ts:13`).
3. `apps/web/app/api/configs/lookup/route.ts:24` has no auth check on the default branch and
   returns every config matching the URL at its latest version. The extension never sends
   `installed=true`. **Install therefore has no effect on what registers**: any package anyone
   published for a site you visit auto-registers, installed or not, account or not.
...
| Version pinning           | claimed; not wired (`installed=true` unused) | real by possession — the body is on disk  |
...
| `index`              | `{ [definitionId]: { domain, urlPatterns, versionId, version, minEngine?, title, installedAt, source } }` | every page load         |
| `pkg:<definitionId>` | the exact `WebMcpConfig` document served at install time                                                  | only on a pattern match |
...
configs that matched, so one fat config can never tax every page load. Key-per-body also means
...
**Validate on read, not just on write.** Run `webMcpConfigSchema` over each body when it is
...
six curated configs (`packages/definitions/configs/*.json`, minified): 0.9–4.7 KB each, mean
...
Revisit if a single config approaches ~100 KB.
...
enforces for config content (`apps/extension/src/lib/engine-gate.ts:16`): never half-run content
...
sequenceDiagram
    ...
    BG->>BG: domainLookupKeys(hostname) → rankConfigsByUrl
...
`rankConfigsByUrl` from `@robertn702/webmcp-cafe-schema` are what
`apps/extension/src/lib/configs.ts:42` already does for the bundled set and what
`apps/web/lib/lookup.ts:31` does server-side. No new matching code — the server-side copy simply
stops being on the hot path.

Concretely:

- **`src/lib/configs.ts`** — `getConfigsForUrl(url)` keeps its signature (so
  `src/lib/register-tools.ts` and its tests are untouched) and its body becomes: read index →
  filter on `domainLookupKeys` → `rankConfigsByUrl` → load + validate bodies → drop revoked.
  `BUNDLED_DOMAINS` and the fallback disappear (§6).
- **`src/lib/registry-client.ts`** — `fetchRegistryConfigs` is deleted. The file keeps only the
...
- **Ranking** loses nothing. `rankConfigsByUrl` sorts on pattern specificity alone
  ...
  (`apps/web/lib/configs-repo.ts:84`). `ARCHITECTURE.md:180` and `mode-flows.ts:77` overstate
...
| `{ v:1, type:"install", definitionId, versionId }` | `{ ok:true, version } \| { ok:false, reason }`                 |
| `{ v:1, type:"uninstall", definitionId }`          | `{ ok:true } \| { ok:false, reason:"not-installed" }`          |
| `{ v:1, type:"list-installs" }`                    | `{ ok:true, installs:[{ definitionId, versionId, version }] }` |

**The extension fetches the body, the page never supplies it.** The page hands over
`{ definitionId, versionId }`; the background fetches it from the registry origin it already
holds host permission for (`apps/extension/wxt.config.ts:34`) and validates with
`webMcpConfigSchema`. Otherwise a compromised or spoofed page could hand the extension a body
...
**New registry endpoint — `GET /api/configs/:id/versions/:versionId`** (public, unauthenticated),
returning the same `webMcpConfigSchema` document at that exact version. Nothing version-scoped
exists today: the only GETs are `/api/configs` (latest), `/api/configs/:id` (latest) and
`/api/configs/lookup` (latest, or the caller's pins). The repo layer is already there —
`getVersionById` (`apps/web/lib/configs-repo.ts:114`) — so the route is thin. Add
`GET /api/configs/:id/versions` (list: version, versionId, changelog, createdAt) at the same
...
- `GET /api/domains` → `{ version, generatedAt, domains: string[] }`, `ETag` + `Cache-Control`,
  derived from `SELECT DISTINCT domain FROM webmcp_definitions`. No identity, no URL, cacheable
  at the edge.
...
over-eager badge. Today there are six configs; this is a list, and stays one.
...
| `definition_id` | FK → `webmcp_definitions.id`                                |
| `version_id`    | FK → `definition_versions.id`, **nullable** = whole package |
...
only at fetch time: `getConfigsForUrl` drops any config whose `definitionId` (or installed
...
stops registering and says why; moving the pin stays the user's explicit act (`POST
/api/configs/:id/update` today, its local equivalent after).
...
`BUNDLED_DOMAINS` (`apps/extension/src/lib/configs.ts:13`) auto-registers five curated configs
...
2. It changes no ordering anywhere. `rankConfigsByUrl` ignores it
   (`packages/schema/src/url-matching.ts:151`), lookup ranks by specificity
   (`apps/web/lib/lookup.ts:33`), browse orders by `updatedAt`
   (`apps/web/lib/configs-repo.ts:84`). It is displayed in three places only:
   `apps/web/app/(registry)/configs/page.tsx:47`, `configs/[id]/page.tsx:37`, and the landing
   hero sum (`apps/web/app/page.tsx:36`).
3. Six configs, zero users. The number carries no information today, and the cheap version
...
`fetchInstallCounts`/`hydrate` (`apps/web/lib/configs-repo.ts:29`) drops out and no consumer
...
With sync deferred, a terminal agent's `install_config` writes an `installs` row that no browser
reads. The three tools (`packages/mcp/src/write-tools.ts:50`, `:63`, `:72`) currently promise an
effect they do not have — the landing copy says so explicitly at
`apps/web/components/landing/mode-flows.ts:83`.

**Recommendation: keep the tools, re-scope the descriptions, and add a handoff link.** Do not
rename them — `@robertn702/webmcp-cafe-mcp` is published, and renaming a tool breaks saved agent
prompts for a cosmetic gain. The description is what the model actually reads, so the
description is what must stop lying:

> `install_config` — Pin this config to a version **on your webmcp.cafe account**. This does not
...
**The handoff link** is the whole fix for Mode 01 in the meantime: `install_config` returns
`https://webmcp.cafe/configs/<id>?install=<versionId>`. The agent prints it, the human clicks
once, the site's Install button runs §3 against that exact `versionId`. That is a one-line
server change plus a query-param branch on the config page, and it makes Mode 01 honest
...
describing `lookup?installed=true`), `:163` (bundled-fallback line), and `:152`/`:157` (per-page
...
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------ |
| 1   | **Version-scoped reads** — `GET /api/configs/:id/versions/:versionId` + `GET /api/configs/:id/versions`, thin over `getVersionById` (`configs-repo.ts:114`)                                                       | `apps/web`                          | None — purely additive, nothing consumes it yet                                      |
...
| 3   | **Extension storage + local page-load path** — `storage` permission, the four keys, `getConfigsForUrl` off the network, `fetchLookup` deleted, bundle demoted to suggestions, revocation enforced at registration | `apps/extension`                    | High: the extension registers nothing until step 4. Separate PR from 4, same release |
...
| 6   | **Drop install count; reword MCP tools; handoff link** — stop populating `installCount`, stats route, browse/detail UI, `write-tools.ts:50`–`:83` descriptions, `install_config` returns the link                 | `apps/web`, `packages/mcp`          | Low, but `packages/mcp` is published — needs a changeset                             |
...
- **Update notifications.** A pinned install never auto-updates, which is the point — but the
  ...
  returns `{ definitionId, latestVersion }` for installed packages… which leaks the install set.
...
picks it up — `chrome.storage.sync` is ~100 KB total and 8 KB per item, so config bodies could
