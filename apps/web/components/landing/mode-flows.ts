...
export const llmFirstSteps: readonly FlowStep[] = [
  ...
  },
  {
    ...
    body: "Postgres serves each package's latest version, ranked by URL-pattern specificity and install count. Nothing there yet? The agent writes one and calls `publish_package`. It gets zod-validated and stored as version 1.",
  },
  {
    from: "api",
    to: "ext",
    title: "Installing pins you to one exact version",
    body: "`install_package` writes an `installs` row pinning your account to a `version_id`. A new upstream version never arrives on its own — moving the pin is the same call with a different `versionId`.",
  },
  {
    from: "ext",
    to: "site",
...
export const inBrowserSteps: readonly FlowStep[] = [
  ...
  },
  {
    ...
    body: "`GET /api/packages/lookup?url=…`, zod-validated at the boundary. Unreachable, no match, or a schema mismatch all fall back to the bundled packages. That happens silently, which is why the page console is the ground truth.",
  },
  {
    from: "api",
    to: "mc",
