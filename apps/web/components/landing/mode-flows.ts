...
export const llmFirstNodes: readonly FlowNode[] = [
  ...
  { id: "api", kicker: "Registry", label: "REST API", detail: "/api/configs/*", col: 2, row: 2 },
  {
    id: "db",
    kicker: "Registry",
    label: "Neon Postgres",
    detail: "definitions · versions · installs",
    col: 2,
    row: 3,
  },
  {
...
export const llmFirstSteps: readonly FlowStep[] = [
  ...
  },
  {
    ...
    body: "`@robertn702/webmcp-cafe-mcp` runs over stdio beside the agent and exposes `lookup_config`, `list_configs`, `upload_config` and `install_config`.",
  },
  {
    from: "mcp",
    to: "api",
    ...
    body: "`GET /api/configs/lookup?url=…` is the same endpoint the extension calls. Writes carry an `Authorization: Bearer` API key you create in settings.",
  },
  {
    from: "api",
    to: "db",
    title: "Newest version, most specific pattern first",
    body: "Postgres serves each definition's latest version, ranked by URL-pattern specificity and install count. Nothing there yet? The agent writes one and calls `upload_config`. It gets zod-validated and stored as version 1.",
  },
  {
    from: "api",
    to: "ext",
    title: "Installing pins you to one exact version",
    body: "`install_config` writes an `installs` row pinning your account to a `version_id`. Your browser asks for `lookup?url=…&installed=true` and gets that pin back. A new upstream version never arrives on its own.",
  },
  {
    from: "ext",
    to: "site",
...
export const inBrowserSteps: readonly FlowStep[] = [
  ...
  },
  {
    ...
    body: "It can't call the registry itself, because the page's CSP would block the fetch. `browser.runtime.sendMessage` passes the URL to the background worker instead.",
  },
  {
    from: "bg",
    to: "api",
    title: "Look up packages for this URL",
    body: "`GET /api/configs/lookup?url=…`, zod-validated at the boundary. Unreachable, no match, or a schema mismatch all fall back to the bundled packages. That happens silently, which is why the page console is the ground truth.",
  },
  {
    from: "api",
    to: "mc",
