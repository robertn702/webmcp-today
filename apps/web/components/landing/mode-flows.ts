...
export const llmFirstSteps: readonly FlowStep[] = [
  {
    ...
    body: "It can load the page, but it has no tools for it — only markup to guess at, and one rename away from breaking.",
  },
  {
    from: "agent",
    to: "mcp",
  ...
  },
  {
    ...
    body: "`GET /api/configs/lookup?url=…` — the same endpoint the extension calls. Writes carry an `Authorization: Bearer` API key you create in settings.",
  },
  {
    from: "api",
    to: "db",
    title: "Newest version, most specific pattern first",
    body: "Postgres serves each definition's latest version, ranked by URL-pattern specificity and install count. Nothing there yet? The agent writes one and calls `upload_config` — zod-validated, stored as version 1.",
  },
  {
    from: "api",
    to: "ext",
...
export const inBrowserSteps: readonly FlowStep[] = [
  {
    ...
    body: "At `document_idle` the extension's content script wakes up on whatever site you're on — signed in, cookies and all.",
  },
  {
    from: "cs",
    to: "bg",
    title: "The content script hands the URL off",
    body: "It can't call the registry itself — the page's CSP would block the fetch — so `browser.runtime.sendMessage` passes the URL to the background worker.",
  },
  {
    from: "bg",
    to: "api",
