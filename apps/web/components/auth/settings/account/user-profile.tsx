...
/** Narrow an unknown session value to a seedable additional-field value. */
function isAdditionalFieldValue(value: unknown): value is AdditionalFieldValue {
  return (
    typeof value === "string" ||
    typeof value === "number" ||
...
export function UserProfile({ className }: UserProfileProps) {
  const { additionalFields, localization } = useAuth();
  ...
  async function handleSubmit(e: SyntheticEvent<HTMLFormElement>) {
    ...
    for (const field of additionalFields ?? []) {
      ...
      const value = parseAdditionalFieldValue(field, formStringOrNull(formData, field.name));
        ...
        <Card className={cn(className)}>
          <CardContent className="flex flex-col gap-6">
            ...
            {additionalFields?.map((field) => {
              ...
              const value: unknown = Reflect.get(session.user, field.name);
              ...
              return (
                <AdditionalField
                  ...
                  field={{
                    ...
                    defaultValue: isAdditionalFieldValue(value) ? value : null,
                  }}
                  isPending={isPending}
                />
              );
