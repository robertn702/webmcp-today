...
/** Narrow a `viewPaths.auth` key to a built-in `AuthView`. */
function isAuthView(key: string): key is AuthView {
  return key in AUTH_VIEWS;
}
...
export function Auth({ className, path, socialLayout, socialPosition, view }: AuthProps) {
  ...
  const authView =
    view ??
    Object.keys(viewPaths.auth)
      .filter(isAuthView)
      .find((key) => viewPaths.auth[key] === path);
