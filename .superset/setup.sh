...
# Provision a fresh Superset workspace for webmcp-today. Idempotent — runs on
# every workspace creation. Superset runs this from a bare, non-interactive
# shell, so don't rely on inherited env vars.
#
# Superset exposes the main repo path as $SUPERSET_ROOT_PATH; fall back to
# deriving it from the git worktree list.

MAIN_REPO="${SUPERSET_ROOT_PATH:-$(git worktree list --porcelain | head -1 | sed 's/^worktree //')}"
...
#    for package @webmcp-today/schema" until it is built. Scoped to
